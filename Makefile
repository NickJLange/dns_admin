...
test: check-env-target
	...
	ssh ${target} cd dns_admin \&\& podman run -d --replace  --name=overlord-dns -p 19000:19000 --env-file=./etc/envfile --dns= -v ./cgi-bin/:/opt/webserver/cgi-bin/ -v ./etc/config.ini:/opt/webserver/etc/config.ini localhost/overlord-dns-admin
