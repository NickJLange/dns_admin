...
#	rsync --progress -rv etc/ ${target}:dns_admin/etc/
#	ssh ${target} cd dns_admin \&\&
	podman run -d --replace --name=overlord-dns -p 19000:19000 --env-file=./etc/envfile --dns= -v ./cgi-bin/:/opt/webserver/cgi-bin/ -v ./lib/:/opt/webserver/lib/  -v ./etc/config.ini:/opt/webserver/etc/config.ini localhost/overlord-dns-admin
